...
(function(){
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    gsap.utils.toArray('[data-reveal]').forEach((el)=>{
      gsap.to(el,{opacity:1,y:0,filter:'blur(0px)',duration:1.1,ease:'luxEase',scrollTrigger:{trigger:el,start:'top 86%',toggleActions:'play none none reverse'}});
    });
    ...
    if(hero){
      ...
      gsap.from('.hero-card',{opacity:0,y:60,filter:'blur(12px)',duration:1.1,delay:.35,ease:'luxEase'});
      gsap.to('.hero-bg img',{scale:1.1,yPercent:8,ease:'none',scrollTrigger:{trigger:hero,start:'top top',end:'bottom top',scrub:true}});
      gsap.to('.hero-portrait img',{yPercent:-10,ease:'none',scrollTrigger:{trigger:hero,start:'top top',end:'bottom top',scrub:true}});
    }
    ...
    if(work && matchMedia('(min-width: 1101px)').matches){
      ...
      items.forEach((item,i)=>{
        ...
        if(img){ tl.to(img,{scale:1.02,filter:'grayscale(.15) brightness(.7) contrast(1.12)',duration:1.2,ease:'none'},i); }
        if(i>0){
          const prevImg = items[i-1].querySelector('.work-img img');
          if(prevImg) tl.to(prevImg,{scale:1.22,filter:'blur(12px) grayscale(1) brightness(.36)',duration:.9,ease:'none'},i-.05);
        }
      });
    }
